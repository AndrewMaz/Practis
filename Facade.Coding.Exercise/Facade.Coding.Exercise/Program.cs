...
namespace Facade.Coding.Exercise
{
    ...
    public class MagicSquareGenerator
    {
        public List<List<int>> Generate(int size)
        {
            ...
            } while (!verifier.Verify(spliter.Split(matrix)));
    ...
    class Program
    {
        public static string ToString(List<List<int>> square)
        {
            var sb = new StringBuilder();
            foreach (var row in square)
            {
                sb.AppendLine(string.Join(" ",
                  row.Select(element => element.ToString())));
            }
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine(ToString(magicSquare.Generate(4)));
        }
    }
}
